...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zGGN spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zGGN serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("GlobalGreenMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("GlobalGreenMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("GlobalGreenMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("globalgreen-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
            ...
            while (chainActive.Tip()->nTime < 1525981707 || vNodes.empty() || pwallet->IsLocked() || !fMintableCoins || (pwallet->GetBalance() > 1000 && nReserveBalance >= pwallet->GetBalance()) || !masternodeSync.IsSynced()) {
        ...
        LogPrintf("Running GlobalGreenMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
