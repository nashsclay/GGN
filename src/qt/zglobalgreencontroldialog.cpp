...
#include "ui_zglobalgreencontroldialog.h"
...
std::list<std::string> ZGlobalGreenControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZGlobalGreenControlDialog::listMints;

ZGlobalGreenControlDialog::ZGlobalGreenControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZGlobalGreenControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZGlobalGreenControlDialog::~ZGlobalGreenControlDialog()
{
    delete ui;
}

void ZGlobalGreenControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGlobalGreenControlDialog::updateList()
{
...
void ZGlobalGreenControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGlobalGreenControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZGlobalGreen_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGlobalGreenControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZGlobalGreenControlDialog::GetSelectedMints()
{
...
void ZGlobalGreenControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
