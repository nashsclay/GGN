...
#include "zglobalgreencontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zGGN ought to be enough for anybody." - Bill Gates, 2017
    ui->zGGNpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzGGNSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzGGN->setEnabled(false);
        ui->pushButtonMintzGGN->setToolTip(tr("zGGN is currently disabled due to maintenance."));

        ui->pushButtonSpendzGGN->setEnabled(false);
        ui->pushButtonSpendzGGN->setToolTip(tr("zGGN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zGGNpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzGGN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGGN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zGGN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zGGN in ") +
...
void PrivacyDialog::on_pushButtonSpendzGGN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGGN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zGGN
        sendzGGN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zGGN
    sendzGGN();
}

void PrivacyDialog::on_pushButtonZGlobalGreenControl_clicked()
{
    ZGlobalGreenControlDialog* zGGNControl = new ZGlobalGreenControlDialog(this);
    zGGNControl->setModel(walletModel);
    zGGNControl->exec();
}

void PrivacyDialog::setZGlobalGreenControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzGGNSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzGGN()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid GlobalGreen Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zGGNpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zGGN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " GLOBALGREEN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zGGN</b>";
    ...
    // use mints from zGGN selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZGlobalGreenControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZGlobalGreenControlDialog::GetSelectedMints();
    }
    ...
    // Clear zglobalgreen selector in case it was used
    ZGlobalGreenControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zGGN Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " GlobalGreen, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zGGN Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zGGN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zGGN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zGGN "));
    ui->labelzGGNAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzGGNSyncStatus->setVisible(fShow);
}
