...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    Object zglobalgreenObj;
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zglobalgreenObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zglobalgreenObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.emplace_back(Pair("zGGNsupply", zglobalgreenObj));
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"globalgreenaddress\"\n"
...
#ifdef ENABLE_WALLET
        // Case 1: GlobalGreen address and we have full public key:
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"globalgreenaddress\" \"signature\" \"message\"\n"
