...
// Copyright (c) 2019 The GlobalGreen developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#if defined(HAVE_CONFIG_H)
#include "config/globalgreen-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("globalgreen-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "globalgreen.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "globalgreend.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the GLOBALGREEN and zGGN money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 21432, 5520));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in GLOBALGREEN/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in GLOBALGREEN/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, globalgreen, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in GLOBALGREEN/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all GlobalGreen specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:21432"));
    ...
    strUsage += HelpMessageOpt("-backupzglobalgreen=<n>", strprintf(_("Enable automatic wallet backups triggered after each zGGN minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizeglobalgreenamount=<n>     " + strprintf(_("Keep N GLOBALGREEN anonymized (default: %u)"), 0) + "\n";
...
{
    RenameThread("globalgreen-loadblk");
...
/** Sanity checks
 *  Ensure that GlobalGreen is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. GlobalGreen Core is shutting down."));
...
#endif
    // Make sure only a single GlobalGreen process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. GlobalGreen Core is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // GlobalGreen: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZGLOBALGREENMinted();
                        RecalculateZGLOBALGREENSpent();
                    }
                    RecalculateGLOBALGREENSupply(1);
                }
                ...
                // GlobalGreen: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of GlobalGreen Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart GlobalGreen Core to complete") << "\n";
        ...
        bool fEnableZGlobalGreenBackups = GetBoolArg("-backupzglobalgreen", true);
        pwalletMain->setZGlobalGreenAutoBackups(fEnableZGlobalGreenBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeGlobalGreenAmount = GetArg("-anonymizeglobalgreenamount", 0);
//    if (nAnonymizeGlobalGreenAmount > 999999) nAnonymizeGlobalGreenAmount = 999999;
//    if (nAnonymizeGlobalGreenAmount < 2) nAnonymizeGlobalGreenAmount = 2;
    ...
    LogPrintf("Anonymize GlobalGreen Amount %d\n", nAnonymizeGlobalGreenAmount);
    ...
    /* Denominations
       ...
       For example:
       1GLOBALGREEN+1000 == (.1GLOBALGREEN+100)*10
       10GLOBALGREEN+10000 == (1GLOBALGREEN+1000)*10
    */
